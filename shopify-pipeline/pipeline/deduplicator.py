"""
deduplicator.py
Compares scraped product URLs against the Shopify export CSV.
Uses the "Variant Metafield: custom.source_url" column to identify
existing products and find NEW ones that need to be scraped.
All URL comparisons are done after normalizing to https://.
"""

import logging
from pathlib import Path
from typing import List, Set, Tuple

import pandas as pd

logger = logging.getLogger(__name__)

SOURCE_URL_COL = "Variant Metafield: custom.source_url"


def find_new_urls(
    scraped_urls: List[str],
    brand: str,
    shopify_exports_dir: str = "shopify_exports",
) -> List[str]:
    """
    Return scraped URLs that do NOT already exist in the Shopify export CSV.

    Args:
        scraped_urls: All URLs collected from the brand website.
        brand: Brand name (used to locate the CSV: {brand}_shopify.csv).
        shopify_exports_dir: Directory containing Shopify export CSVs.

    Returns:
        List of URLs not yet in Shopify (new products to scrape).
    """
    existing = _load_shopify_urls(brand, shopify_exports_dir)
    logger.info(f"Shopify export has {len(existing)} existing source URLs.")

    new_urls = [
        url for url in scraped_urls
        if _normalize_url(url) not in existing
    ]
    logger.info(
        f"Scraped {len(scraped_urls)} URLs → {len(new_urls)} are new "
        f"({len(scraped_urls) - len(new_urls)} already in Shopify)."
    )
    return new_urls


def find_removed_products(
    scraped_urls: List[str],
    brand: str,
    shopify_exports_dir: str = "shopify_exports",
) -> pd.DataFrame:
    """
    Find Shopify products whose source URLs no longer exist on the brand website.
    Used in DRAFTING mode.

    Args:
        scraped_urls: All URLs currently live on the brand website.
        brand: Brand name.
        shopify_exports_dir: Directory containing Shopify export CSVs.

    Returns:
        DataFrame of rows from the Shopify CSV for removed products
        (one row per product — the primary row with Handle and Title).
    """
    csv_path = _get_csv_path(brand, shopify_exports_dir)
    if not csv_path.exists():
        logger.warning(f"No Shopify export found at {csv_path}. Returning empty.")
        return pd.DataFrame()

    df = _load_csv(csv_path)

    live_normalized = {_normalize_url(u) for u in scraped_urls}

    # Work only with rows that have a source_url set
    mask = df[SOURCE_URL_COL].notna() & (df[SOURCE_URL_COL].str.strip() != "")
    df_with_url = df[mask].copy()

    # Identify removed: source_url no longer live
    removed_mask = df_with_url[SOURCE_URL_COL].apply(
        lambda u: _normalize_url(str(u)) not in live_normalized
    )
    removed_df = df_with_url[removed_mask].copy()

    # Deduplicate to one row per Handle
    if not removed_df.empty and "Handle" in removed_df.columns:
        removed_df = removed_df.drop_duplicates(subset=["Handle"], keep="first")

    logger.info(f"Found {len(removed_df)} Shopify products no longer on website.")
    return removed_df


# ---------------------------------------------------------------------------
# Internal helpers
# ---------------------------------------------------------------------------

def _load_shopify_urls(brand: str, shopify_exports_dir: str) -> Set[str]:
    """Load and normalize all source URLs from the Shopify export CSV."""
    csv_path = _get_csv_path(brand, shopify_exports_dir)
    if not csv_path.exists():
        logger.warning(
            f"Shopify export not found: {csv_path}. "
            "Treating all scraped URLs as new."
        )
        return set()

    df = _load_csv(csv_path)

    if SOURCE_URL_COL not in df.columns:
        logger.warning(
            f"Column '{SOURCE_URL_COL}' not found in {csv_path}. "
            "Treating all scraped URLs as new."
        )
        return set()

    urls = df[SOURCE_URL_COL].dropna().astype(str)
    return {_normalize_url(u) for u in urls if u.strip()}


def _load_csv(path: Path) -> pd.DataFrame:
    """Load a CSV with flexible encoding fallback."""
    for enc in ("utf-8", "utf-8-sig", "latin-1"):
        try:
            return pd.read_csv(path, encoding=enc, dtype=str, low_memory=False)
        except UnicodeDecodeError:
            continue
    raise ValueError(f"Could not read CSV with any supported encoding: {path}")


def _get_csv_path(brand: str, exports_dir: str) -> Path:
    return Path(exports_dir) / f"{brand}_shopify.csv"


def _normalize_url(url: str) -> str:
    """Normalize URL to https:// for consistent comparison."""
    url = url.strip()
    if url.startswith("//"):
        url = "https:" + url
    elif url.startswith("http://"):
        url = "https://" + url[7:]
    # Remove trailing slash for consistency
    return url.rstrip("/")
